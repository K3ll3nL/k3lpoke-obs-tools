...
// Measured native loudness of the clip, shown in the UI so the user always
// knows the source level. Normalization itself lives in the envelope.
export function setClipLoudness(id, lufs) {
  const clip = data.clips.find(c => c.id === id)
  if (!clip) return null
  clip.loudness_lufs = lufs != null && isFinite(lufs) ? Number(lufs) : null
  save()
  return { loudness_lufs: clip.loudness_lufs }
}

// Marks that the current envelope was generated by auto-normalize (UI state).
export function setClipNormalized(id, enabled) {
  const clip = data.clips.find(c => c.id === id)
  if (clip) { clip.normalized = !!enabled; save() }
}
